...
def make_preprocessor(df: pd.DataFrame, dense: bool = False):
    ...
    if dense:
        ...
        try:
            ohe = OneHotEncoder(handle_unknown="ignore")  # sparse_output=True by default on new versions
        except TypeError:
...
def load_adult_local(csv_path: str) -> pd.DataFrame:
    ...
    # Strip whitespace ONLY on object columns (no applymap → avoids pandas deprecation)
    obj_cols = df.select_dtypes(include=["object"]).columns
    if len(obj_cols) > 0:
        df[obj_cols] = df[obj_cols].apply(lambda s: s.str.strip())

    # Convert '?' to NaN on object columns and drop rows with missing values (simple baseline)
    for col in obj_cols:
...
def main(args):
    ...
    # Save artifacts (encoders & each model pipeline)
    joblib.dump(le, os.path.join(args.artifacts, "label_encoder.joblib"))
    # Save a default preprocessor (not required by app, but kept for completeness)
